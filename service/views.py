...
from .tasks import assync_process_data, sync_process_data, split_data, reconstruct_data
...
def secretshare_split(request):
    ...
    try:
        response = split_data(data)
    except:
        response = {"Message": "An error occurred while processing the data."}
...
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def secretshare_reconstruct(request):
    ...
    try:
        response = reconstruct_data(data)
    except:
        response = {"Message": "An error occurred while processing the data."}
